...
import { Tetromino, generateDefaultData, initColor, toLeft, toRight, toRotate } from "./function";
...
export const Playfield: React.FC<PlayfieldProps> = ({ gameStart, gameReset }) => {
    ...
    useFrame(() => {
        if (ref.current) {
            const { playfield } = gameData.current
            let instanceId = 0;
            for (let i = 0; i < 15; i++) {
                for (let j = 0; j < 10; j++) {
                    const shape = playfield[i][j]
                    if (shape) {
                        let color: Float32Array = colors[shape as keyof typeof colors];
                        ref.current.geometry.attributes.color.setXYZW(
                            instanceId,
                            color[0],
                            color[1],
                            color[2],
